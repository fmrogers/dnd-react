...
/**
 * Places an item immediately before the target item (not after like placeItemsBeforeTarget)
 */
export function placeItemsBeforeTargetActually<T extends { children?: T[] }, K extends keyof T>(
  items: T[],
  idKey: K,
  draggedItemIdsPath: T[K][],
  ...
  // Calculate the correct insertion index for BEFORE target
  let insertIndex = targetLoc.index;

  // If moving within the same parent and the dragged item was before the target,
  // we need to adjust the insertion index because removal shifted everything
  if (sameParent && draggedLoc.index < targetLoc.index) {
    insertIndex = targetLoc.index - 1; // Insert one position earlier because we removed an item before target
  }

  // Insert the dragged item at the calculated position (BEFORE target)
  targetLoc.parent.splice(insertIndex, 0, draggedItem);
